...
def get_data():
    train, test = get_dataset('data_/mhealth.dat')
...
def main():
    ...
    BOOLEAN_STRANFER = True

    if BOOLEAN_STRANFER:
        print('STRANFER_LEARNING')
        pretrained_dict = torch.load("all_net_pam.pth")  # 加载参数
        # 过滤操作
        new_dict = {}
    ...
    torch.save(net.state_dict(), "pamap2_mhealth_net_pam.pth") # 保存参数
